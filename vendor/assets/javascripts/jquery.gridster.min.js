...
function (t, i, e) {
    function s(i, e, s) {
        this.options = t.extend(r, s), this.$element = i, this.last_colliders = [], this.last_colliders_coords = [], this.set_colliders(e), this.init()
    }
    var r = {
    ...
    o.init = function () {
        ...
        for (var h = 0, n = i.length; n > h; h++) - 1 === t.inArray(i[h], r) && s.call(this, i[h])
    }, o.find_collisions = function (i) {
        for (var e = this, s = this.options.overlapping_region, r = [], o = [], a = this.colliders || this.$colliders, h = a.length, n = e.$element.coords().update(i || !1).get(); h--;) {
            var _ = e.$colliders ? t(a[h]) : a[h],
                d = _.isCoords ? _ : _.coords(),
                l = d.get(),
                c = e.overlaps(n, l);
            if (c) {
                var p = e.detect_overlapping_region(n, l);
                if (p === s || "all" === s) {
                    var g = e.calculate_overlapped_area_coords(n, l),
...
function (t) {
    t.delay = function (t, i) {
        ...
        var e, s, r, o, a, h, n = debounce(function () {
            a = o = !1
        }, i);
        return function () {
            e = this, s = arguments;
            var _ = function () {
                r = null, a && t.apply(e, s), n()
            };
            return r || (r = setTimeout(_, i)), o ? a = !0 : h = t.apply(e, s), n(), o = !0, h
        }
    }
}(window),
function (t, i, e) {
    ...
    d.init = function () {
        ...
        this.calculate_dimensions(), this.$container.css("position", "static" === e ? "relative" : e), this.disabled = !1, this.events(), t(i).bind("resize.gridster-draggable", throttle(t.proxy(this.calculate_dimensions, this), 200))
    }, d.events = function () {
    ...
    }, d.get_actual_pos = function (t) {
        var i = t.position();
        return i
    }, d.get_mouse_pos = function (t) {
        if (h) {
    ...
    }, d.get_offset = function (t) {
        t.preventDefault();
        var i = this.get_mouse_pos(t),
            e = Math.round(i.left - this.mouse_init_pos.left),
            s = Math.round(i.top - this.mouse_init_pos.top),
            r = Math.round(this.el_init_offset.left + e - this.baseX + this.scroll_offset_x),
            o = Math.round(this.el_init_offset.top + s - this.baseY + this.scroll_offset_y);
        return this.options.limit && (r > this.player_max_left ? r = this.player_max_left : this.player_min_left > r && (r = this.player_min_left)), {
            ...
            pointer: {
                left: i.left,
                top: i.top,
                diff_left: e + this.scroll_offset_x,
                diff_top: s + this.scroll_offset_y
            }
        }
    }, d.get_drag_data = function (t) {
        var i = this.get_offset(t);
        return i.$player = this.$player, i.$helper = this.helper ? this.$helper : this.$player, i
    }, d.set_limits = function (t) {
    ...
    }, d.scroll_in = function (i, s) {
        var r, h = a[i],
            n = 50,
            d = 30,
            l = "x" === i,
            c = l ? this.window_width : this.window_height,
            p = l ? t(e).width() : t(e).height(),
            g = l ? this.$player.width() : this.$player.height(),
            u = o["scroll" + _(h)](),
            f = u,
            w = f + c,
            m = w - n,
            ...
            v = f + s.pointer[h],
            z = p - c + g;
        return v >= m && (r = u + d, z > r && (o["scroll" + _(h)](r), this["scroll_offset_" + i] += d)), y >= v && (r = u - d, r > 0 && (o["scroll" + _(h)](r), this["scroll_offset_" + i] -= d)), this
    }, d.manage_scroll = function (t) {
        this.scroll_in("x", t), this.scroll_in("y", t)
    }, d.calculate_dimensions = function () {
        this.window_height = o.height(), this.window_width = o.width()
    }, d.drag_handler = function (i) {
        if (i.target.nodeName, !this.disabled && (1 === i.which || h) && !this.ignore_drag(i)) {
            var e = this,
                s = !0;
    ...
    }, d.on_dragstart = function (t) {
        if (t.preventDefault(), this.is_dragging) return this;
        this.drag_start = this.is_dragging = !0;
        var i = this.$container.offset();
        return this.baseX = Math.round(i.left), this.baseY = Math.round(i.top), this.initial_container_width = this.options.container_width || this.$container.width(), "clone" === this.options.helper ? (this.$helper = this.$player.clone().appendTo(this.$container).addClass("helper"), this.helper = !0) : this.helper = !1, this.scroll_offset_y = 0, this.scroll_offset_x = 0, this.el_init_offset = this.$player.offset(), this.player_width = this.$player.width(), this.player_height = this.$player.height(), this.set_limits(this.options.container_width), this.options.start && this.options.start.call(this.$player, t, this.get_drag_data(t)), !1
    }, d.on_dragmove = function (t) {
...
function (t, i, e) {
    ...
    o.init = function () {
    ...
    }, o.add_widget = function (i, e, s, r, o, a, h) {
        var n;
        e || (e = 1), s || (s = 1), !r & !o ? n = this.next_position(e, s) : (n = {
            col: r,
            row: o
    ...
    }, o.set_widget_min_size = function (t, i) {
    ...
    }, o.mutate_widget_in_gridmap = function (i, e, s) {
        e.size_x;
        var r = e.size_y,
        ...
        t.each(o.cols, function (i, e) {
            -1 === t.inArray(e, a.cols) && h.push(e)
        });
        var n = [];
        t.each(a.cols, function (i, e) {
            -1 === t.inArray(e, o.cols) && n.push(e)
        });
        var _ = [];
        ...
        }), this.remove_from_gridmap(e), n.length) {
        ...
        if (e.col = s.col, e.row = s.row, e.size_x = s.size_x, e.size_y = s.size_y, this.add_to_gridmap(s, i), i.removeClass("player-revert"), i.data("coords").update({
            ...
            var p = [h[0], s.row, h.length, Math.min(r, s.size_y), i];
    ...
    }, o.register_widget = function (i) {
        var e = {
            col: parseInt(i.attr("data-col"), 10),
            row: parseInt(i.attr("data-row"), 10),
            size_x: parseInt(i.attr("data-sizex"), 10),
            size_y: parseInt(i.attr("data-sizey"), 10),
            max_size_x: parseInt(i.attr("data-max-sizex"), 10) || !1,
            max_size_y: parseInt(i.attr("data-max-sizey"), 10) || !1,
            min_size_x: parseInt(i.attr("data-min-sizex"), 10) || !1,
            min_size_y: parseInt(i.attr("data-min-sizey"), 10) || !1,
            el: i
        };
        return this.options.avoid_overlapped_widgets && !this.can_move_to({
            size_x: e.size_x,
            size_y: e.size_y
        }, e.col, e.row) && (t.extend(e, this.next_position(e.size_x, e.size_y)), i.attr({
            "data-col": e.col,
            "data-row": e.row,
            "data-sizex": e.size_x,
            "data-sizey": e.size_y
        })), i.data("coords", i.coords()), i.data("coords").grid = e, this.add_to_gridmap(e, i), this.options.resize.enabled && this.add_resize_handle(i), this
    ...
    }, o.on_start_resize = function (i, e) {
        ...
        var e, s = i.pointer.diff_left,
            ...
            n = this.options.widget_margins[1],
            ...
            g = 1 / 0,
            u = 1 / 0,
            f = Math.ceil(s / (o + 2 * h) - .2),
            w = Math.ceil(r / (a + 2 * n) - .2),
            ...
            z = v * this.min_widget_width - 2 * h;
        if (m = Math.max(Math.min(m, _), d), m = Math.min(v, m), e = _ * o + 2 * (m - 1) * h, g = Math.min(e, z), min_width = d * o + 2 * (m - 1) * h, y = Math.max(Math.min(y, l), c), u = l * a + 2 * (y - 1) * n, min_height = c * a + 2 * (y - 1) * n, this.resize_dir.right ? y = this.resize_initial_sizey : this.resize_dir.bottom && (m = this.resize_initial_sizex), p) {
        ...
        var h = this.get_widgets_overlapped(this.player_grid_data),
            n = this.widgets_constraints(h);
        if (this.manage_movements(n.can_go_up, o, a), this.manage_movements(n.can_not_go_up, o, a), !h.length) {
        ...
        return i.each(t.proxy(function (i, e) {
            var a = t(e),
                h = a.coords().grid;
            this.can_go_widget_up(h) ? (s = s.add(a), r.push(h)) : o.push(h)
        }, this)), e = i.not(s), {
        ...
        return t.each(i, t.proxy(function (t, i) {
            ...
            if (a) this.move_widget_to(o, a), this.set_placeholder(e, a + r.size_y);
                ...
                if (!h) {
                    var n = s + this.player_grid_data.size_y - r.row;
                    this.move_widget_down(o, n), this.set_placeholder(e, s)
                }
            }
        }, this)), this
        ...
        var a = e > this.placeholder_grid_data.row,
            h = this.placeholder_grid_data.col !== i;
        this.placeholder_grid_data.col = i, this.placeholder_grid_data.row = e, this.cells_occupied_by_placeholder = this.get_cells_occupied(this.placeholder_grid_data), this.$preview_holder.attr({
            "data-row": e,
            "data-col": i
        }), (a || h) && r.each(t.proxy(function (e, r) {
        ...
        n.length && n.each(t.proxy(function (i, r) {
            var o = t(r);
        ...
        return this.for_each_column_occupied(t, function (t) {
            var a = this.gridmap[t],
                h = i + 1;
            for (s[t] = []; --h > 0 && (this.is_empty(t, h) || this.is_player(t, h) || this.is_widget(t, h) && a[h].is(o));) s[t].push(h), r = r > h ? h : r;
        ...
        return this.for_each_column_occupied(t, function (o) {
            ...
            s[o] = [];
            for (var h = i + 1; --h > 0 && (!this.is_widget(o, h) || this.is_player_in(o, h) || a[h].is(t.el));) this.is_player(o, h) || this.is_placeholder_in(o, h) || this.is_player_in(o, h) || s[o].push(h), r > h && (r = h);
            return 0 === s[o].length ? (e = !1, !0) : (s[o].sort(function (t, i) {
                return t - i
            }), undefined)
        }), e ? this.get_valid_rows(t, s, r) : !1
    }, o.get_valid_rows = function (i, e, s) {
        for (var r = i.row, o = i.row + i.size_y - 1, a = i.size_y, h = s - 1, n = []; o >= ++h;) {
            var _ = !0;
            if (t.each(e, function (i, e) {
                t.isArray(e) && -1 === t.inArray(h, e) && (_ = !1)
            }), _ === !0 && (n.push(h), n.length === a)) break
        }
        var d = !1;
        return 1 === a ? n[0] !== r && (d = n[0] || !1) : n[0] !== r && (d = this.get_consecutive_numbers_index(n, a)), d
        ...
        return e || (e = 1), this.can_go_up(i) ? (this.for_each_column_occupied(s, function (s) {
            if (-1 === t.inArray(i, o)) {
                var a = i.coords().grid,
                    h = r - e;
                if (h = this.can_go_up_to_row(a, s, h), !h) return !0;
                var n = this.widgets_below(i);
                this.remove_from_gridmap(a), a.row = h, this.add_to_gridmap(a), i.attr("data-row", a.row), this.$changed = this.$changed.add(i), o.push(i), n.each(t.proxy(function (i, s) {
        ...
        if (-1 === t.inArray(i, o)) {
            ...
            this.remove_from_gridmap(h), _.each(t.proxy(function (i, e) {
                var s = t(e),
                    r = s.coords().grid,
                    o = this.displacement_diff(r, h, a);
                o > 0 && this.move_widget_down(s, o)
            }, this)), h.row = n, this.update_widget_position(h, i), i.attr("data-row", h.row), this.$changed = this.$changed.add(i), o.push(i)
        }
        ...
        if (this.for_each_column_occupied(i, function (t) {
            for (o[t], h[t] = [], r = n; r-- && this.is_empty(t, r) && !this.is_placeholder_in(t, r);) h[t].push(r);
            return h[t].length ? undefined : (a = !1, !0)
        }), !a) return !1;
        for (r = s, r = 1; n > r; r++) {
            for (var _ = !0, d = 0, l = h.length; l > d; d++) h[d] && -1 === t.inArray(r, h[d]) && (_ = !1);
            if (_ === !0) {
                a = r;
                break
            }
        ...
        var r = t.el,
            ...
            a = !0,
            h = i + t.size_x - 1;
        return h > this.cols ? !1 : s && e + t.size_y - 1 > s ? !1 : (this.for_each_cell_occupied(o, function (i, e) {
        ...
        return e
    }, o.get_cells_occupied = function (i) {
        var e, s = {
            cols: [],
            rows: []
        };
            ...
            var l = [],
                ...
                p = {
                    "for_each/above": function () {
                        for (; c-- && !(c > 0 && this.is_widget(s, c) && -1 === t.inArray(a[s][c], l) && (h = o.call(a[s][c], s, c), l.push(a[s][c]), h)););
                    },
                    "for_each/below": function () {
                        for (c = r + 1, n = a[s].length; n > c && (!this.is_widget(s, c) || -1 !== t.inArray(a[s][c], l) || (h = o.call(a[s][c], s, c), l.push(a[s][c]), !h)); c++);
                    }
                };
            p[_] && p[_].call(this)
        }
    ...
    }, o.set_dom_grid_height = function (t) {
        if (t === undefined) {
    ...
    }, o.set_dom_grid_width = function (t) {
        t === undefined && (t = this.get_highest_occupied_cell().col);
        ...
        for (this.generated_stylesheets.push(a), s.generated_stylesheets.push(a), e = i.cols; e >= 0; e--) r += i.namespace + ' [data-col="' + (e + 1) + '"] { left:' + (e * i.widget_base_dimensions[0] + e * i.widget_margins[0] + (e + 1) * i.widget_margins[0]) + "px; }\n";
        ...
        var i = s.generated_stylesheets,
            e = this.generated_stylesheets;
        this.$style_tags.remove(), s.generated_stylesheets = t.map(i, function (i) {
            return -1 === t.inArray(i, e) ? i : undefined
        })
        ...
        this.cols = Math.max(a, r, this.options.min_cols), 1 / 0 !== s && s >= a && this.cols > s && (this.cols = s);
        var h = this.options.extra_rows;
        return this.$widgets.each(function (i, e) {
            h += +t(e).attr("data-sizey")
        }), this.rows = Math.max(h, this.options.min_rows), this.baseX = (t(i).width() - e) / 2, this.baseY = this.$wrapper.offset().top, this.options.autogenerate_stylesheet && this.generate_stylesheet(), this.generate_faux_grid(this.rows, this.cols)
    }, o.destroy = function (e) {
        return this.$el.removeData("gridster"), t(i).unbind(".gridster"), this.drag_api && this.drag_api.destroy(), this.remove_style_tags(), e && this.$el.remove(), this
